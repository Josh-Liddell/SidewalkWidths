...
RUN python3.6 -m venv pipeline/Road-Segmentation/venv_py36 && \
    ./pipeline/Road-Segmentation/venv_py36/bin/pip install --upgrade pip && \
    if [ -f requirements_py36.txt ]; then ./pipeline/Road-Segmentation/venv_py36/bin/pip install -r requirements_py36.txt; fi
